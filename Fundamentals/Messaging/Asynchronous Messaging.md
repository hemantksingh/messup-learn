...
You can drive RabbitMQ, Kafka or Azure Service Bus with their raw clients. A bus library (NServiceBus, MassTransit, Rebus, Wolverine, CAP) sits on top and saves you writing the same code again:

* the outbox and idempotent-consumer plumbing that turn at-least-once delivery into "effectively once" (see above);
