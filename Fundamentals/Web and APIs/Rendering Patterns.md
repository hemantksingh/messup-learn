...
## Middleware pipelines: OWIN and ASP.NET Core

OWIN (Open Web Interface for .NET) defined a standard boundary between a web framework and the server hosting it, so an ASP.NET application could run outside IIS, in a console process or on Linux. Katana was Microsoft's implementation. ASP.NET Core made the idea native: Kestrel hosts the app and requests flow through an ordered pipeline of middleware, each of which can act and pass the request on. OWIN itself is no longer needed, though an interop package exists. The pattern matters because it is the same shape in every modern web stack (Express, Koa, Rack): a request is a value passed through a chain of functions.
