...
Phase 1 (cut, merge, move) ran on 2026-09-16, straight after the baseline commit. Every path named in this folder is **pre-migration**; `phase1-moves.md` maps each old path to where its content now lives. Phase 2 (correct the KEEP pages) ran the same day: 33 pages, every High and Medium WRONG, STALE, INCONSISTENT and BROKEN finding applied, opinion attributed where the source was known. Per-page "was → now" logs are in `details/phase2/`. Phase 3 (the fifteen rewrites) ran the same day: each page rewritten from the question it answers, the owner's own sentences and examples kept, every audited error gone, sources named; then tightened to the owner's rule of brief, plain text with no AI slop. Per-page reports, the shared brief and the tighten brief are in `details/phase3/`; each report ends with the diagrams Phase 5 should draw. Phase 4 (trim and split) ran the same day: seven trims (Observability, Load Balancing and Proxies, Cloud Security, Endpoint Security, Data Platforms, Infrastructure as Code, the AWS hub) and four splits (Web Application Risks; Code Quality into Code Quality and Functional and Reactive Programming; HTTP into HTTP and Realtime Web; Asynchronous Messaging into Messaging Fundamentals, Sagas and Process Managers, and Message Brokers), plus a new Resilience Patterns page. Every page is now under the 3,000-word prose ceiling. Reports in `details/phase4/`. Phase 5 (wire up) ran the same day: H1s aligned to filenames, frontmatter on all 63 pages, root README index and AGENTS.md generated, 37 diagrams drawn as draw.io XML and embedded as `images/*.drawio.svg` with the source inside, every raster image and `.drawio` file removed, 345 links checked with 32 replaced and 6 removed, Data Privacy corrected. Structural check clean; longest page 2,949 words. Report and manifests in `details/phase5/`.
