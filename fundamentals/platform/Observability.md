...
[SRE](https://sre.google/) is Google's name for running production systems with an engineering discipline: reliability is a feature with a target, and the target is what lets teams decide how much risk to take. Ideas from it that this page depends on:

* A **service level objective** (SLO) is the reliability target you promise yourself; the **error budget** is the gap between the SLO and perfection. While the budget is intact the team ships freely; when it is spent, reliability work takes priority. This is what dissolves the tension between "ship more" and "break less".
