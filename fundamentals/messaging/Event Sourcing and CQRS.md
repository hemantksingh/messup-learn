...
Event Store is an appending file therefore it can be used as a queue as well as a persistent store for all your events that capture the system state. In order to achieve guaranteed message delivery, event driven systems may use a distributed transaction that involves a 2 phase commit of the event once to the event storage and once to the persistent queue. Having the Event Store act as a queue removes the latency of writing the event to the Event Store and the queue in the same transaction. This allows us to avoid distributed transactions like DTC which may provide consistency but at the cost of availability. Consumers then have to track their own position in the store; this is the same idea as the Transactional Outbox described in [Asynchronous Messaging](Asynchronous%20Messaging.md#domain-and-message-persistence-consistency).

Messaging systems with at-least-once delivery require event handlers to be idempotent or to deduplicate by event id; see [Asynchronous Messaging](Asynchronous%20Messaging.md#producers-guarantee-at-least-once-delivery-and-consumers-implement-idempotency).
